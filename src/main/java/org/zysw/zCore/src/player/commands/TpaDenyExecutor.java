...
public class TpaDenyExecutor implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        requester.sendMessage(ConvertStringToComponent.convertWithComponents(ZCore.getInstance().getConfig().getString("tpa-settings.tpadeny.requester-message"), requesterPlaceholders));

        // target
        Map<String, Component> targetPlayerPlaceholders = Map.of(
                "requester", Component.text(requester.getName())
        );
        target.sendMessage(ConvertStringToComponent.convertWithComponents(ZCore.getInstance().getConfig().getString("tpa-settings.tpadeny.target-player-message"), targetPlayerPlaceholders));
